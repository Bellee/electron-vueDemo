<template>
    ...
    <el-table
        ...
        <template slot-scope="scope">
          <p class="operate">
            上传
            <input type="file" ref="uploaFile" @change="getfilePath(scope.$index, scope.row,scope.$event)"/>
          </p>
        </template>
        </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    getfilePath(index, row, $event) {
      ...
      let fileTitle = _this.fileWait[index].projectName + '-' + _this.fileWait[index].sets
      console.log(fileTitle)
      episode.addItem(_this.filePath, fileTitle, _this.fileWait[index].projectEpisodeId)
      _this.fileWait.splice(index, 1)
    ...
    getFileList() {
      var _this = this
      episode.getEpisodeList(this.producerId, 1, this.name, this.cPage, this.csize, function(items) {
        console.log(items)
