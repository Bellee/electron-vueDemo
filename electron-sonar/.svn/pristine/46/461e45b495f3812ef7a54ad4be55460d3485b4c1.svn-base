...
var updateOrigin = (item, callback) => {
  db.replaceTable('mv_origin', '_id', [item], callback)
}
...
// 更新剧集
module.exports.updateOrigin = (item, callback) => {
  updateOrigin(item, callback)
}

// 查询音频
module.exports.selectNovideo = (projectEpisodeId, callback) => {
  var cond = {}
  cond.project_episode_id = projectEpisodeId
  db.query('mv_novideos', cond, callback)
}

module.exports.novideo = (row, progressCallback, callback) => {
  if (row.is_pause == '1') {
    callback(null, null)
  } else if (row.pause_status > 1) {
    var cond = {}
    cond.project_episode_id = row.project_episode_id
    db.query('mv_novideos', cond, callback)
  } else {
    updateOrgStatus(row.file_title, '1', '1')
    progressCallback(row.project_episode_id, 1, 1)
    // 删除旧acrid
    var cond1 = {}
    cond1.file_title = row.file_title
    db.queryOne('mv_novideos', cond1, (err, org) => {
      if (org && org.acr_id) {
        acrcloud.deleteAudio({
          endpoint: '/v1/audios/' + org.acr_id
        }, (err, httpResponse, body) => {
          console.log('删除旧acrid:' + org.acr_id + ' ' + httpResponse.statusCode)
        })
      }
    })
    async.waterfall([
...
module.exports.uploadACRCloud = (org, row, progressCallback, callback) => {
  if (org.is_pause == '1') {
    callback(null, null)
  } else if (org.pause_status > 2) {
    callback(null, null)
  } else {
  ...
  acrcloud.deleteAudio({
    ...
    if (err) {
      ...
      if (body.status == 500) {
        // 内部错误
        row.del_status = -1
        db.updateTable('mv_novideos', '_id', [row], callback)
...
module.exports.resize = (row, progressCallback, callback) => {
  if (row.is_pause == '1') {
    callback(null, null)
  } else if (row.pause_status > 3) {
    var cond = {}
    cond.project_episode_id = row.project_episode_id
    db.query('mv_resize', cond, callback)
  } else {
...
// 查询切片
module.exports.selectCut = (projectEpisodeId, callback) => {
  var cond = {}
  cond.project_episode_id = projectEpisodeId
  db.query('mv_cut', cond, callback)
}

module.exports.cut = (org, row, progressCallback, callback) => {
  if (org.is_pause == '1') {
    callback(null, null)
  } else if (org.pause_status > 4) {
    var cond = {}
    cond.project_episode_id = row.project_episode_id
    ...
    async.waterfall([
      ...
      (metadata, callback) => {
        ...
        if (metadata.format) {
          ...
          // console.log(item)
          db.deleteTable('mv_cut', {
            'file_origin_name': row.file_name
          }, (err, rows) => {
            // 去重过滤 file_name
            // var fileNames = rows.map(row => row.file_name)
            // opts = opts.filter(item => fileNames.indexOf(item.file_name) == -1)
            async.mapLimit(opts, scheduleOptions.cut.cutlimit, (opt, callback) => {
              cut(opt, opts.length, progressCallback, callback)
            }, (err, results) => {
              // 全部切割完毕后更新resize文件状态
              row.cut_status = 1
              row.updated_at = new Date()
...
module.exports.uploadQiniu = (org, row, length, progressCallback, callback) => {
  if (org.is_pause == '1') {
    callback(null, null)
  } else {
...
module.exports.uploadQiniuComplete = (org, row, progressCallback, callback) => {
  if (org.is_pause == '1') {
    callback(null, null)
  } else {
    // 更新上传状态
